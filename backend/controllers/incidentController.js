...
import Incident from "../models/Incident.js";
import User from "../models/User.js"; // ✅ Remove Responder import

// ✅ Volunteers can take incidents now
export const createIncident = async (req, res) => {
  try {
    ...
    const incident = new Incident({
      ...
      severity,
      createdBy,
      assignedVolunteer: null, // ✅ No responders, volunteers will take incidents
    });
...
// ✅ Volunteers can take an incident
export const takeIncident = async (req, res) => {
  try {
    ...
    if (incident.assignedVolunteer) {
      return res.status(400).json({ message: "Incident already taken" });
    }

    incident.assignedVolunteer = volunteerId;
    await incident.save();

    await User.findByIdAndUpdate(volunteerId, { $addToSet: { assignedIncidents: incidentId } });
...
// ✅ Remove responder-related function
export const getIncidentsByLocation = async (req, res) => {
  try {
    const { location } = req.query;
    const incidents = await Incident.find({ location });
