...
import Shelter from './../models/Shelter.js';
import { loadSheltersFromCSV } from '../utils/loadSheltersFromCSV.js'; // ✅ New import
...
// ✅ New function to load shelters from CSV
export const getSheltersFromCSV = async (req, res) => {
    try {
        const shelters = await loadSheltersFromCSV();
        res.status(200).json(shelters);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Failed to load shelters from CSV", error: error.message });
    }
};
